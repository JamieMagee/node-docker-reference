...
import { validateDigest, isDigest } from './digest';
...
export function parseQualifiedNameOptimized(name: string) {
...
export function parseFamiliarName(
  name: string,
...
export function parseAll(name: string): Reference {
  if (anchoredIdentifierRegexp.test(name)) {
